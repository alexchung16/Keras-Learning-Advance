...
class DenseNet():
    def __init__(self, input_shape, num_classes=2, num_filters=64, growth_k=32, num_dense_block=4, num_layers=None,
                 reduction=0.0, keep_prob=0.0, weight_decay=1e-4, batch_norm_epsilon=1e-5):
    ...
    def inference(self):
        predict = self.dense_net(inputs=self.inputs, num_classes=self.num_classes, num_filters=self.num_filters,
                               ...
                               grows_k=self.growth_k, keep_prob=self.keep_prob,
                               batch_norm_epsilon=self.batch_norm_epsilon, compression=self.compression)

        model = Model(inputs=self.inputs, outputs=predict)
        return model

    def dense_net(self, inputs, num_classes, num_filters, num_dense_block, num_layers, grows_k, keep_prob,
                  batch_norm_epsilon=1e-5, compression=1.0):
        # 224 x 224 x 3
        x = ZeroPadding2D(padding=(3, 3), name='conv1_zeropadding')(inputs)
        # 230 x 230 x 3
        x = Convolution2D(filters=num_filters, kernel_size=(7, 7), strides=(2, 2), padding='valid', use_bias=False,
                          name='conv1')(x)
        # 111 x 111 x 3
        x = Scale(axis=3, name='conv1_scale')
        # 111 x 111 x 3
        ...
        for block_id in range(num_dense_block - 1):
            ...
            x, num_filters = self.dense_block(inputs=x, stage=stage, num_layers=num_layers[block_id],
                                              number_filters=num_filters, growth_k=grows_k, keep_prob=keep_prob,
        ...
        x, num_filters = self.dense_block(inputs=x, stage=final_stage, num_layers=num_layers[-1],
                                          number_filters=num_filters, growth_k=grows_k, keep_prob=keep_prob,
                                          batch_norm_epsilon=batch_norm_epsilon)
        x = BatchNormalization(epsilon=batch_norm_epsilon, name='conv' + str(final_stage)+'_blk_bn')
        x = Scale(axis=self.concat_axis, name=str(final_stage)+'_blk_scale')(x)
        x = Activation(activation='relu', name=str(final_stage)+'_blk_scale')(x)
        x = GlobalAveragePooling2D(name='pool'+str(final_stage))(x)

        x = Dense(units=num_classes, name='fc6')(x)
        predict = Activation(activation='predict')(x)

        return predict

    def bottleneck_block(self, inputs, stage, branch, num_filters, keep_prob=None, batch_norm_epsilon=1e-5):
        """
    ...
    def dense_block(self, inputs, stage, num_layers, number_filters, growth_k, keep_prob=None, batch_norm_epsilon=1e-5,
                    growth_filters=True):
        """
        ...
        for n in range(num_layers):
            ...
            x = self.bottleneck_block(inputs=concat_feat, stage=stage, branch=branch, num_filters=growth_k,
                                      keep_prob=keep_prob, batch_norm_epsilon=batch_norm_epsilon)
    ...
    def transition_block(self, inputs, stage, num_filters, compression=1.0, keep_prob=None, batch_norm_epsilon=1e-5):
        """
        ...
        x = Convolution2D(filters=(num_filters*compression), kernel_size=(1, 1), strides=(1, 1), use_bias=False,
